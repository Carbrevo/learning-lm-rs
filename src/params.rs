...
macro_rules! hidden_tensors {
    ($safetensor:ident, $class:ident, $field:ident, $ttype:ty, $config:ident) => {{
        ...
        for layer in 0..$config.num_hidden_layers as i32 {
            let tensorview = $safetensor
                .tensor($class(layer, stringify!($field)).as_str())
                .unwrap();
            let blen = View::data_len(&tensorview);
            let tensor_data = (0..blen / size_of::<$ttype>())
                .map(|x| {
                    let data = tensorview.data();
                    Convert::<$ttype>::convert(
                        &data[x * size_of::<$ttype>()..(x + 1) * size_of::<$ttype>()],
                    )
                })
                .collect();
            tensors.push(Tensor::<$ttype>::new(
                tensor_data,
                &Vec::from(tensorview.shape()),
            ));
        }
        tensors
    }};
        ...
        let tensorview = $safetensor
            ...
            .or_else(|e| {
                if $config.tie_word_embeddings {
                    let name = format!(
                        "{}.weight",
                        ["lm_head", "embedding_table"]
                            .iter()
                            .find(|x| { **x != stringify!($class) })
                            .unwrap()
                    );
